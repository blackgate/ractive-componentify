...
module.exports = function build (source, result, options) {
  ...
  return Promise.all([compiledJS, compiledStyles]).then(function (res) {
    ...
    if (partials.length > 0) {
      script.push("component.exports.partials = component.exports.partials || {};");
      script.push("var partials = {\n" + partials.join(',\n') + "\n};");
      script.push(
        "for (var k in partials) {\n  " +
          "if (partials.hasOwnProperty(k))\n    " +
            "component.exports.partials[k] = partials[k];\n" +
        "}"
      );
    }
